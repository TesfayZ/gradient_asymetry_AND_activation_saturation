...
Recent work on gradient imbalance in multi-task RL~\cite{yu2020gradient} shows that tasks producing larger gradients can dominate optimization, potentially starving other objectives. Chen et al.~\cite{chen2021gradient} introduced gradient normalization techniques to balance conflicting gradients in multi-objective settings. Research on Distributional Soft Actor-Critic~\cite{ma2020dsac} identified that high variance in critic gradients can cause training instability, particularly with different reward scales.
