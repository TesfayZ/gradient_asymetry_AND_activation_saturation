...
\textbf{Implications for Hyperparameter Search:} The fact that only 1 out of 16 (6.25\%) learning rate combinations converged represents a significant practical barrier. In the thesis experiments, 93.75\% of hyperparameter configurations failed---not due to suboptimal learning dynamics, but because actors stopped updating entirely due to activation saturation. This has profound implications for computational costs: exhaustive hyperparameter search wastes resources on configurations that are fundamentally incapable of learning. \textit{Preventing activation saturation would transform many of these failed configurations into viable candidates}, dramatically reducing hyperparameter search costs and making actor-critic algorithms more accessible to practitioners with limited computational budgets.
...
\begin{itemize}
    ...
    \item Add gradient clipping on critic updates (see Section~\ref{sec:discussion} for why actor clipping is ineffective)
...
\subsection{Network Capacity Does Not Prevent Saturation (Preliminary)}
...
0.01 & 0.001 & 253 & 244 \\
0.01 & 0.0001 & 265 & 253 \\
0.001 & 0.1 & 595 & \textbf{2000} \\
0.0001 & 0.1 & 1326 & \textbf{2000} \\
\bottomrule
\end{tabular}
\end{table}
