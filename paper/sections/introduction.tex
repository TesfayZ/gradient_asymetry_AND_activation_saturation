...
In our prior work on the Client-Master MADRL framework (CCM-MADRL) for MEC task offloading~\cite{ccm_madrl, gebrekidan2024thesis}, we conducted hyperparameter tuning experiments involving 16 combinations of learning rates. During this investigation, we observed a striking phenomenon: \textbf{client agents (actors) exhibited weight update cessation}---their neural network parameters stopped changing within $\sim$5 episodes under high learning rate configurations, while the master agent (critic) continued learning until training completion. Figure~\ref{fig:thesis_stopping} illustrates this phenomenon from our original experiments.

The severity of this issue becomes apparent when examining convergence across multiple runs. In the original thesis experiments~\cite{gebrekidan2024thesis} (Chapter~5), which used a different random seed and environment configuration from the current work, 10 independent runs were conducted for each of the 16 learning rate combinations with 95\% confidence intervals. The results showed that \textbf{only 1 of 16 configurations consistently achieved policy improvement across all runs}---specifically, the configuration with actor LR = 0.0001 and critic LR = 0.001. All other configurations either exhibited premature actor stopping or showed high variance in learning outcomes, with confidence intervals spanning performance improvements to policy degradation.
