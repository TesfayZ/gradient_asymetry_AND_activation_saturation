...
Our analysis has limitations that suggest directions for future research. The experiments were conducted on a single domain (MEC task offloading); generalization to other continuous control environments (e.g., MuJoCo, robotic manipulation) requires validation. Our preliminary gradient clipping experiments confirm that clipping addresses critic gradient explosion but cannot resolve actor gradient vanishing caused by tanh saturation. Future work should explore alternative bounded activations with better gradient properties (e.g., softsign, scaled sigmoid) and develop adaptive learning rate schemes that account for the measured gradient asymmetry.
