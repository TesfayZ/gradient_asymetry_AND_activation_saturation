...
\textbf{Controlled Variables:} To isolate learning rate effects, we fixed all random seeds (seed = 42) for reproducibility: PyTorch (including CUDA), NumPy, Python random, and environment seeds. We also set \texttt{torch.backends.cudnn.deterministic = True} and disabled cuDNN benchmarking. Our initial experiments were conducted without seed control; we subsequently re-ran all 16 configurations with fixed seeds to ensure reproducibility. The seeded results are used throughout this paper. The unseeded run exhibited the same structural patterns---identical stopping behavior for high actor learning rates and the same 6 of 16 configurations reaching full training---but with higher reward variance in converging configurations, underscoring the stochastic sensitivity of these algorithms even when the qualitative behavior is robust.
...
\textbf{Reproducibility:} Code and experimental logs are available at \url{https://github.com/TesfayZ/gradient_asymetry_AND_activation_saturation}. Results are from single seeded runs per configuration; future work should include multiple seeds for statistical significance testing.
