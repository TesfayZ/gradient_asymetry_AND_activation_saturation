"""
Input Normalization Experiment - Testing RunningNorm on inputs ONLY

Hypothesis: Normalizing inputs alone (without hidden layer normalization) may
prevent activation saturation by controlling the scale of values entering the network.

This approach should:
1. Reduce input scale variance that contributes to pre-activation explosion
2. Isolate the effect of input normalization from hidden layer normalization
3. Be faster than FullNorm (no LayerNorm overhead on hidden layers)

Learning rate combinations:
- Actor LRs: [0.0001, 0.001, 0.01, 0.1] (all)
- Critic LRs: [0.0001, 0.001, 0.01, 0.1] (all)
...
for _path in ['/content/inputnorm_experiment',
              os.path.join(os.path.dirname(os.path.abspath(__file__)), 'inputnorm_experiment')]:
...
try:
    ...
    print("DEBUG: inputnorm_experiment exists:", os.path.exists('/content/inputnorm_experiment'))
    if os.path.exists('/content/inputnorm_experiment'):
        print("DEBUG: inputnorm_experiment contents:", os.listdir('/content/inputnorm_experiment'))
    raise

# KEY CHANGE: Import InputNormActorNetwork and InputNormCriticNetwork
from Model import InputNormActorNetwork, InputNormCriticNetwork
...
class Config:
    ...
    # All learning rate combinations
    CLIENT_LRS = [0.0001, 0.001, 0.01, 0.1]  # All actor LRs
    MASTER_LRS = [0.0001, 0.001, 0.01, 0.1]  # All critic LRs
    ...
    # Directories - Colab paths (UPDATED for inputnorm)
    RESULTS_DIR = '/content/results/inputnorm_experiment'
    DRIVE_BACKUP_DIR = '/content/drive/MyDrive/inputnorm_results'


class InputNormExperimentAgent:
    """CCM_MADDPG with InputNorm actor/critic and tracking for stopping episode detection."""
    ...
    def _init_networks(self):
        ...
        # KEY CHANGE: Use InputNormActorNetwork with RunningNorm on input only
        self.actors = [
            InputNormActorNetwork(self.state_dim, self.action_dim, torch.tanh)
        ...
        # KEY CHANGE: Use InputNormCriticNetwork with RunningNorm on input only
        self.critics = [InputNormCriticNetwork(critic_state_dim, critic_action_dim,
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'inputnorm_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"InputNorm Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"{'='*60}")

    agent = InputNormExperimentAgent(actor_lr, critic_lr, exp_dir, Config.USE_CUDA)
...
def run_all_experiments():
    """Run all 16 experiments with InputNorm actor/critic."""
    # Set seed for reproducibility
    set_seed(Config.SEED)
    print(f"Random seed set to: {Config.SEED}")

    print("="*70)
    print("INPUT NORMALIZATION EXPERIMENT")
    print("Testing RunningNorm on inputs ONLY (NO hidden layer normalization)")
    print("="*70)
    print(f"Actor LRs: {Config.CLIENT_LRS}")
    print(f"Critic LRs: {Config.MASTER_LRS}")
