"""
Gradient Clipping Experiment - Testing if gradient clipping prevents training instability

Hypothesis: Gradient clipping limits the extreme gradient magnitudes, potentially:
1. Stabilizing critic updates that have gradients ~10^7
2. Preventing explosive weight updates in early training
3. May not directly address tanh saturation (root cause of vanishing actor gradients)
...
- max_grad_norm = 1.0 (standard value, clips gradient norm to 1)
...
- Tracks gradient norms before and after clipping
"""
...
for _path in ['/content/gradient_clipping_experiment',
              os.path.join(os.path.dirname(os.path.abspath(__file__)), 'gradient_clipping_experiment')]:
...
try:
    ...
    print("DEBUG: gradient_clipping_experiment exists:", os.path.exists('/content/gradient_clipping_experiment'))
    if os.path.exists('/content/gradient_clipping_experiment'):
        print("DEBUG: gradient_clipping_experiment contents:", os.listdir('/content/gradient_clipping_experiment'))
    raise

# Standard ActorNetwork (no LayerNorm - same as baseline)
...
class Config:
    ...
    # GRADIENT CLIPPING - KEY PARAMETER
    MAX_GRAD_NORM = 1.0  # Standard value used in PPO, TD3, etc.
    ...
    # Directories - Colab paths
    RESULTS_DIR = '/content/results/gradient_clipping_experiment'
    DRIVE_BACKUP_DIR = '/content/drive/MyDrive/gradient_clipping_results'


class GradientClippingExperimentAgent:
    """CCM_MADDPG with gradient clipping and tracking for stopping episode detection."""

    def __init__(self, actor_lr, critic_lr, exp_dir, max_grad_norm=1.0, use_cuda=True):
        self.actor_lr = actor_lr
        self.critic_lr = critic_lr
        self.exp_dir = exp_dir
        self.max_grad_norm = max_grad_norm  # KEY: Gradient clipping threshold
        ...
        self.clipping_history = []  # Track how often clipping occurs
    ...
    def _init_networks(self):
        ...
        # Standard ActorNetwork (no LayerNorm)
        self.actors = [
            ActorNetwork(self.state_dim, self.action_dim, torch.tanh)
            for _ in range(self.n_agents)
        ]

        critic_state_dim = self.n_agents * self.state_dim
        critic_action_dim = self.n_agents * self.action_dim
        self.critics = [CriticNetwork(critic_state_dim, critic_action_dim,
    ...
    def train(self):
        """Training with gradient clipping applied to both actor and critic."""
    ...
    def train_with_tracking(self):
        """Train with gradient tracking before and after clipping."""
        if self.n_episodes <= self.episodes_before_train:
            return

        # Track gradients BEFORE clipping (during normal train call)
        # We need a modified training loop for detailed tracking
        self._train_with_gradient_tracking()
    ...
    def _train_with_gradient_tracking(self):
        """Training with detailed gradient tracking before/after clipping."""
        if self.n_episodes <= self.episodes_before_train:
            return
        ...
        # ==================== CRITIC UPDATE WITH GRADIENT TRACKING ====================
        critic_loss = torch.nn.MSELoss()(current_q, target_q)
        critic_loss.requires_grad_(True)
        self.critics_optimizer[0].zero_grad()
        critic_loss.backward()
        ...
        critic_grad_before = 0.0
        for param in self.critics[0].parameters():
            if param.grad is not None:
                critic_grad_before += param.grad.data.norm(2).item() ** 2
        critic_grad_before = np.sqrt(critic_grad_before)
        ...
        critic_grad_after = 0.0
        for param in self.critics[0].parameters():
            if param.grad is not None:
                critic_grad_after += param.grad.data.norm(2).item() ** 2
        critic_grad_after = np.sqrt(critic_grad_after)

        self.critics_optimizer[0].step()
        self._soft_update_target(self.critics_target[0], self.critics[0])

        # Track clipping for critic
        critic_was_clipped = bool(critic_grad_before > self.max_grad_norm) if self.max_grad_norm else False

        # ==================== ACTOR UPDATE WITH GRADIENT TRACKING ====================
        actor_grad_before_sum = 0.0
        actor_grad_after_sum = 0.0
        actor_clipped_count = 0
        ...
        for agent_id in range(self.n_agents):
            ...
            # Track gradient norm BEFORE clipping
            actor_grad_before = 0.0
            for param in self.actors[agent_id].parameters():
                if param.grad is not None:
                    actor_grad_before += param.grad.data.norm(2).item() ** 2
            actor_grad_before = np.sqrt(actor_grad_before)
            actor_grad_before_sum += actor_grad_before
            ...
            for param in self.actors[agent_id].parameters():
                if param.grad is not None:
                    actor_grad_after += param.grad.data.norm(2).item() ** 2
            actor_grad_after = np.sqrt(actor_grad_after)
            actor_grad_after_sum += actor_grad_after

            if actor_grad_before > self.max_grad_norm if self.max_grad_norm else False:
                actor_clipped_count += 1

            self.actors_optimizer[agent_id].step()
            self._soft_update_target(self.actors_target[agent_id], self.actors[agent_id])

        # Log clipping statistics periodically
        if self.n_episodes % Config.GRADIENT_LOG_INTERVAL == 0:
            self.clipping_history.append({
                'episode': int(self.n_episodes),
                'critic_grad_before': float(critic_grad_before),
                'critic_grad_after': float(critic_grad_after),
                'critic_was_clipped': critic_was_clipped,
                'actor_avg_grad_before': float(actor_grad_before_sum / self.n_agents),
                'actor_avg_grad_after': float(actor_grad_after_sum / self.n_agents),
                'actor_clipped_count': int(actor_clipped_count),
                'actor_clipped_ratio': float(actor_clipped_count / self.n_agents),
            })
    ...
    def save_results(self):
        ...
        tracking = {
            ...
            'clipping_history': self.clipping_history,
        }
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'gradclip_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"Gradient Clipping Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"max_grad_norm={Config.MAX_GRAD_NORM}")
    print(f"{'='*60}")

    agent = GradientClippingExperimentAgent(
        actor_lr, critic_lr, exp_dir,
...
def run_all_experiments():
    """Run all 16 experiments with gradient clipping."""
    print("="*70)
    print("GRADIENT CLIPPING EXPERIMENT")
    print("Testing if gradient clipping (max_grad_norm=1.0) prevents training failure")
    print("="*70)
