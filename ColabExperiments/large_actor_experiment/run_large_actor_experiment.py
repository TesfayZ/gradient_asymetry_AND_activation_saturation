"""
Large Actor Experiment - Testing if larger actor architecture prevents saturation

Hypothesis: Making the actor have the same hidden layer sizes as the critic
(512 -> 128 instead of 64 -> 32) may help avoid tanh saturation.

Key changes from original experiment:
1. Actor uses LargeActorNetwork (7 -> 512 -> 128 -> 3) instead of (7 -> 64 -> 32 -> 3)
2. Experiments run from HIGH to LOW learning rates to prioritize early-stopping cases
...
try:
    ...
    print("DEBUG: large_actor_experiment exists:", os.path.exists('/content/large_actor_experiment'))
    if os.path.exists('/content/large_actor_experiment'):
        print("DEBUG: large_actor_experiment contents:", os.listdir('/content/large_actor_experiment'))
    raise
from Model import LargeActorNetwork, CriticNetwork
...
class StoppingExperimentAgent:
    ...
    def _init_networks(self):
        ...
        # KEY CHANGE: Use LargeActorNetwork (512->128) instead of ActorNetwork (64->32)
        self.actors = [
            LargeActorNetwork(self.state_dim, self.action_dim, torch.tanh)
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'large_actor_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"LARGE ACTOR Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"{'='*60}")
...
def run_all_experiments():
    ...
    print("\n" + "="*70)
    print("LARGE ACTOR EXPERIMENTS COMPLETE")
