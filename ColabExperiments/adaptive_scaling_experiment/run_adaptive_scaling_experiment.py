...
Maps [G_min, G_max] -> [-2, +2] before tanh, so tanh(±2) ≈ ±0.96.
- tanh'(±2) ≈ 0.07 — viable gradient even at extremes
- Full output range (-0.96, +0.96) is utilized without saturation
- ±1 only reachable when learning pushes beyond tracked range (genuine commitment)

Key Parameters:
- target_range = 2.0: Maps [G_min, G_max] -> [-2, +2], so tanh(±2) ≈ ±0.96
- Gradient clipping: max_norm=1.0 on both actor and critic
- Strict min/max tracking: Only expands, never shrinks
- Per-dimension tracking: Each action can independently commit to boundaries
...
- Tracks G_min/G_max evolution, output uniformity, and scaling statistics
"""
...
for _path in ['/content/adaptive_scaling_experiment',
              os.path.join(os.path.dirname(os.path.abspath(__file__)), 'adaptive_scaling_experiment')]:
...
try:
    ...
    print("DEBUG: adaptive_scaling_experiment exists:", os.path.exists('/content/adaptive_scaling_experiment'))
    if os.path.exists('/content/adaptive_scaling_experiment'):
        print("DEBUG: adaptive_scaling_experiment contents:", os.listdir('/content/adaptive_scaling_experiment'))
    raise

from Model import AdaptiveUniformTanhActor, CriticNetwork
...
class Config:
    ...
    # ADAPTIVE SCALING - KEY PARAMETER
    TARGET_RANGE = 2.0  # Maps [G_min, G_max] -> [-2, +2], tanh(±2) ≈ ±0.96, tanh'(±2) ≈ 0.07
    ...
    SCALING_LOG_INTERVAL = 10  # Track G_min/G_max evolution
...
class AdaptiveScalingExperimentAgent:
    """CCM_MADDPG with Adaptive Uniform Tanh Scaling and tracking."""

    def __init__(self, actor_lr, critic_lr, exp_dir, target_range=2.0, use_cuda=True):
        ...
        # NEW: Adaptive scaling specific tracking
        self.scaling_history = []  # Track G_min, G_max evolution
    ...
    def _init_networks(self):
        ...
        # Use AdaptiveUniformTanhActor instead of standard ActorNetwork
        self.actors = [
            AdaptiveUniformTanhActor(self.state_dim, self.action_dim,
    ...
    def _register_hooks(self):
        """Register hooks to capture pre-activations at fc3 layer."""
    ...
    def _log_activation_stats(self):
        """Log pre-activation statistics and compute saturation ratio."""
    ...
    def _log_scaling_stats(self):
        """Log adaptive scaling statistics (G_min, G_max, uniformity)."""
        # Aggregate scaling stats from all actors
        all_g_min = []
        all_g_max = []

        for actor in self.actors:
            stats = actor.get_scaling_stats()
            all_g_min.append(stats['g_min'])
            all_g_max.append(stats['g_max'])

        # Average across agents
        avg_g_min = np.mean(all_g_min, axis=0).tolist()
        avg_g_max = np.mean(all_g_max, axis=0).tolist()
        avg_g_range = (np.array(avg_g_max) - np.array(avg_g_min)).tolist()

        return {
            'episode': self.n_episodes,
            'avg_g_min': avg_g_min,
            'avg_g_max': avg_g_max,
            'avg_g_range': avg_g_range,
            'per_agent_g_min': all_g_min[:5],  # First 5 agents for sampling
            'per_agent_g_max': all_g_max[:5],
        }
    ...
    def train(self):
        ...
        torch.nn.utils.clip_grad_norm_(self.critics[0].parameters(), 1.0)
    ...
    def train_with_tracking(self):
        """Train with gradient, activation, and scaling tracking."""
        self.train()
        ...
        # NEW: Scaling tracking (G_min, G_max evolution)
        if self.n_episodes % Config.SCALING_LOG_INTERVAL == 0:
            scaling_stats = self._log_scaling_stats()
            self.scaling_history.append(scaling_stats)
    ...
    def save_results(self):
        ...
        tracking = {
            ...
            'scaling_history': self.scaling_history,  # NEW
        }
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'adaptive_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"Adaptive Scaling Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"target_range={Config.TARGET_RANGE}")
    print(f"{'='*60}")

    agent = AdaptiveScalingExperimentAgent(
...
def run_all_experiments():
    """Run all 16 experiments with adaptive scaling."""
    print("="*70)
    print("ADAPTIVE UNIFORM TANH SCALING EXPERIMENT")
    print("Adaptive scaling with target_range=2.0 and gradient clipping (max_norm=1.0)")
    print("="*70)
