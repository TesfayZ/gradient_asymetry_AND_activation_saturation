...
class MecEnv(object):
    ...
    def step_mec(self, action):
        ...
        Time_proc =  self.S_size*8*1024*1024*self.S_cycle / (CAPABILITY_E*10**9)
        Time_local = self.S_size*8*1024*1024*self.S_cycle / (A_res)
        Time_max_local = self.S_size*8*1024*1024*self.S_cycle / (MIN_RES*10**9)
        Time_off = self.S_size*8*1024*1024/ DataRate 
        ...
        # Energy model: E = k * C * f^2 where C = total cycles, f = frequency
        # S_size is in MB, converted to bits: MB * 8 * 1024 * 1024
        Energy_local = K_ENERGY_LOCAL * self.S_size*8*1024*1024*self.S_cycle* (A_res**2)
        Energy_max_local = K_ENERGY_LOCAL * self.S_size*8*1024*1024*self.S_cycle* ((self.S_res*10**9)**2) 
