"""
Layer Normalization Experiment (CONTINUED) - Remaining 14 experiments

Hypothesis: LayerNorm normalizes pre-activations to ~0 mean, ~1 std before tanh,
keeping outputs in the linear region where gradients flow properly.

ALREADY COMPLETED (will be skipped):
- Actor 0.1, Critic 0.1: Stopped at 632
- Actor 0.1, Critic 0.01: Full 2000 episodes!
...
- Remaining: 12 experiments (excluding 4 completed)
...
for _path in ['/content/layernorm_experiment_continued',
              os.path.join(os.path.dirname(os.path.abspath(__file__)), 'layernorm_experiment_continued')]:
...
try:
    ...
    print("DEBUG: layernorm_experiment_continued exists:", os.path.exists('/content/layernorm_experiment_continued'))
    if os.path.exists('/content/layernorm_experiment_continued'):
        print("DEBUG: layernorm_experiment_continued contents:", os.listdir('/content/layernorm_experiment_continued'))
    raise

# KEY CHANGE: Import LayerNormActorNetwork instead of ActorNetwork
from Model import LayerNormActorNetwork, CriticNetwork
...
class LayerNormExperimentAgent:
    """CCM_MADDPG with LayerNorm actor and tracking for stopping episode detection."""
    ...
    def _init_networks(self):
        ...
        # KEY CHANGE: Use LayerNormActorNetwork with LayerNorm before tanh
        self.actors = [
            LayerNormActorNetwork(self.state_dim, self.action_dim, torch.tanh)
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'layernorm_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def load_status():
    # Pre-populated with already completed experiments from previous runs
    ALREADY_COMPLETED = [
        'layernorm_0.1_critic_0.1_run_0',
        'layernorm_0.1_critic_0.01_run_0',
        'layernorm_0.1_critic_0.001_run_0',
        'layernorm_0.1_critic_0.0001_run_0',
    ]
    ...
    if os.path.exists(status_file):
        with open(status_file, 'r') as f:
            status = json.load(f)
        # Merge with pre-completed
        for exp in ALREADY_COMPLETED:
            if exp not in status['completed']:
                status['completed'].append(exp)
        return status
    return {'completed': ALREADY_COMPLETED.copy(), 'in_progress': None}
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"LayerNorm Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"{'='*60}")

    agent = LayerNormExperimentAgent(actor_lr, critic_lr, exp_dir, Config.USE_CUDA)
...
def run_all_experiments():
    """Run remaining 14 experiments with LayerNorm actor."""
    print("="*70)
    print("LAYER NORMALIZATION EXPERIMENT (CONTINUED)")
    print("Testing if LayerNorm before tanh prevents saturation")
    print("="*70)
    print("ALREADY COMPLETED (will be skipped):")
    print("  - Actor 0.1, Critic 0.1: Stopped at 632")
    print("  - Actor 0.1, Critic 0.01: Full 2000 episodes!")
    print("="*70)
    print(f"Actor LRs (HIGH to LOW): {Config.CLIENT_LRS}")
    print(f"Critic LRs (HIGH to LOW): {Config.MASTER_LRS}")
    print(f"Remaining experiments: 14 (of 16 total)")
