"""
Linear Activation Experiment - Testing if linear hidden activations prevent saturation

Hypothesis: ReLU only outputs positive values [0, +inf), but rewards are negative.
Using linear hidden activations allows the network to naturally represent negative
values throughout the computation.

Key changes from original experiment:
1. Actor uses LinearActorNetwork with LINEAR hidden activations (no ReLU)
2. Critic uses LinearCriticNetwork with LINEAR hidden activations (no ReLU)
3. Both networks use large architecture (512 -> 128)
4. Experiments run from HIGH to LOW learning rates to prioritize early-stopping cases
...
# Add linear_activation_experiment to path - Colab environment
for _path in ['/content/linear_activation_experiment',
...
try:
    ...
    print("DEBUG: linear_activation_experiment exists:", os.path.exists('/content/linear_activation_experiment'))
    if os.path.exists('/content/linear_activation_experiment'):
        print("DEBUG: linear_activation_experiment contents:", os.listdir('/content/linear_activation_experiment'))
    raise
from Model import LinearActorNetwork, LinearCriticNetwork
...
class Config:
    ...
    # Directories - Colab paths (separate from original experiment)
    RESULTS_DIR = '/content/results/linear_activation_experiment'
    DRIVE_BACKUP_DIR = '/content/drive/MyDrive/linear_activation_results'


class StoppingExperimentAgent:
    """CCM_MADDPG with tracking for stopping episode detection."""
    ...
    def _init_networks(self):
        ...
        # KEY CHANGE: Use LinearActorNetwork with LINEAR hidden activations (no ReLU)
        self.actors = [
            LinearActorNetwork(self.state_dim, self.action_dim, torch.tanh)
            for _ in range(self.n_agents)
        ]

        critic_state_dim = self.n_agents * self.state_dim
        critic_action_dim = self.n_agents * self.action_dim
        # KEY CHANGE: Use LinearCriticNetwork with LINEAR hidden activations (no ReLU)
        self.critics = [LinearCriticNetwork(critic_state_dim, critic_action_dim,
        ...
        # Print architecture info for this experiment
        actor_params = sum(p.numel() for p in self.actors[0].parameters())
        critic_params = sum(p.numel() for p in self.critics[0].parameters())
        print(f"\n*** LINEAR ACTIVATION EXPERIMENT ***")
        print(f"Actor: {self.state_dim} -> 512 -> 128 -> {self.action_dim} (LINEAR hidden, tanh output)")
        print(f"Critic: 510 -> 512 -> 128 -> 1 (LINEAR hidden, linear output)")
...
def get_experiment_key(actor_lr, critic_lr, run=0):
    return f'linear_{actor_lr}_critic_{critic_lr}_run_{run}'
...
def run_single_experiment(actor_lr, critic_lr, run=0):
    ...
    print(f"\n{'='*60}")
    print(f"LINEAR ACTIVATION Experiment: actor_lr={actor_lr}, critic_lr={critic_lr}")
    print(f"{'='*60}")

    agent = StoppingExperimentAgent(actor_lr, critic_lr, exp_dir, Config.USE_CUDA)
...
def run_all_experiments():
    ...
    print("="*70)
    print("LINEAR ACTIVATION EXPERIMENT - Testing if linear hidden layers prevent saturation")
    print("="*70)
    ...
    print(f"Order: High LR first (0.1, 0.1) -> ... -> (0.0001, 0.0001)\n")
    ...
    print("\n" + "="*70)
    print("LINEAR ACTIVATION EXPERIMENTS COMPLETE")
